  ...
  for (const type of typesToTry) {
    try {
      ...
      const meta = {
        ...
        type: type, season: parsed.season, episode: parsed.episode
      };
...
// ===================
// CALLED BY KOTLIN when cineby URL changes
// ===================
window.onCinebyUrlChange = async function(url) {
  if (!url || !url.includes('cineby')) return;
...
// Called by Kotlin when user presses HOME button in cineby view
window.onReturnHome = async function() {
  await refreshHomeData();
};

// ===================
// OPEN CINEBY (via native WebView now, not iframe)
// ===================
function openCineby(url) {
  const targetUrl = url || startUrl;
  Nav.openUrl(targetUrl);
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // The webview bar buttons are now native Android views
  // but we keep the home screen nav links working
  ...
  // Nav tab buttons on home screen
